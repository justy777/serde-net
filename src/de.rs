...
/// # Errors
pub fn from_reader<'a, T, U>(input: &'a mut T) -> Result<U> where T: AsRef<[u8]>, U: Deserialize<'a> {
...
impl<'de, 'a, T: AsRef<[u8]>> de::Deserializer<'de> for &'a mut Deserializer<'de, T> {
    ...
    #[allow(clippy::cast_possible_truncation)]
    fn deserialize_tuple_struct<V>(self, _name: &'static str, len: usize, visitor: V) -> Result<V::Value> where V: Visitor<'de> {
        let value = visitor.visit_seq(LengthDefined::new(self, len as u16))?;
        Ok(value)
    }
    ...
    fn deserialize_struct<V>(self, _name: &'static str, fields: &'static [&'static str], visitor: V) -> Result<V::Value> where V: Visitor<'de> {
...
impl<'a, 'de, T: AsRef<[u8]>> de::SeqAccess<'de> for LengthDefined<'a, 'de, T> {
    ...
    fn next_element_seed<U>(&mut self, seed: U) -> Result<Option<U::Value>> where U: DeserializeSeed<'de> {
...
impl <'a, 'de, T: AsRef<[u8]>> de::VariantAccess<'de> for Enum<'a, 'de, T> {
    ...
    fn newtype_variant_seed<U>(self, seed: U) -> Result<U::Value> where U: DeserializeSeed<'de> {
